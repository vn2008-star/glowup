...
export async function POST(request: Request) {
  ...
  try {
    switch (action) {
      ...
      // ─── DASHBOARD OVERVIEW ───
      // Shared implementation with the server-rendered /dashboard page —
      // this action remains for client-side refetches.
      case 'dashboard.overview': {
        const overview = await getDashboardOverview(svc, tenantId)
