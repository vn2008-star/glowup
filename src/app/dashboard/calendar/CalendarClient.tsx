...
export type InitialCalendarData = {
  /** UTC range the server prefetched (default week view around salon-tz today) */
  startISO: string;
  endISO: string;
  staff: Staff[];
  services: Service[];
  appointments: FullAppointment[];
  birthdays: Pick<Client, "id" | "first_name" | "last_name" | "birthday">[];
};
...
export default function CalendarClient({ initialCalendar }: { initialCalendar: InitialCalendarData | null }) {
  const { tenant } = useTenant();
  const salonTz = tenant?.timezone || DEFAULT_TZ;
  const t = useTranslations("calendarPage");
  const [view, setView] = useState<"day" | "week" | "month">("week");
  const [selectedDate, setSelectedDate] = useState(new Date());
  // Server-prefetched week: paint immediately. If the browser's idea of "this
  // week" differs from the server's (timezone edge), the first fetch effect
  // detects the range mismatch and refetches.
  const [appointments, setAppointments] = useState<FullAppointment[]>(initialCalendar?.appointments ?? []);
  const [staffMembers, setStaffMembers] = useState<Staff[]>(initialCalendar?.staff ?? []);
  const [services, setServices] = useState<Service[]>(initialCalendar?.services ?? []);
  ...
  const [initialLoaded, setInitialLoaded] = useState(!!initialCalendar);
  const initialRangeConsumed = useRef(false);
  ...
  const fetchCalendarData = useCallback(async () => {
    ...
    // First run with server-prefetched data: if it covers exactly this range,
    // the fetch is redundant — skip it. A mismatch falls through and refetches.
    if (initialCalendar && !initialRangeConsumed.current) {
      initialRangeConsumed.current = true;
      if (initialCalendar.startISO === startISO && initialCalendar.endISO === endISO) {
        setLoading(false);
        return;
      }
    }
